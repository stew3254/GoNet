...
func main() {
	...
	if !listen {
		...
		if udp {
			// TODO fix this
			laddr, err := net.ResolveUDPAddr("udp", net.JoinHostPort(ip, strconv.Itoa(sport)))
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				return
			}
			client.Con, _ = net.ListenUDP("udp", laddr)
			// Wait until a client sends something to know how to connect back to them
			b := make([]byte, 4096)
			_, raddr, _ := client.Con.(*net.UDPConn).ReadFromUDP(b)
			// TODO Remember to fix this when client.In/Out matters more
			fmt.Print(string(b))
			client.Con.Close()
			client.Con, err = net.DialUDP("udp", laddr, raddr)
		} else {
